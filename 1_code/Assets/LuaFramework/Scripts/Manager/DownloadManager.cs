...
public class DownloadManager : Manager
{
	...
	void Update()
	{
		if (gameMgr != null)
		{
			if (gameMgr.IsHangUp())
			{
				if (gameMgr.IsUpdDebug())
					Debug.Log("[Download][OnLineUpdDebug] Application is HangUp");
				return;
			}


			if (gameMgr.IsNetDetailDebug())
			{
				StringBuilder sb = new StringBuilder();
				sb.Append(" netstate:" + m_NetReachable);
				sb.Append(" working:" + m_DownloadingTask.Count + ", waiting:" + m_WaitingTask.Count + "\n");
				foreach (KeyValuePair<string, Task> kv in m_DownloadingTask)
					sb.Append(kv.Key + ":" + kv.Value.result + ":" + kv.Value.IsUrlOverflow() + "\n");
				Debug.Log("[Download][OnLineNetDebug] " + sb.ToString());
			}
		}
		...
		if (gameMgr != null && gameMgr.IsNetDebug())
			Debug.Log(string.Format("[Download][OnLineNetDebug] netsate({0}) working({1}) waiting({2})", m_NetReachable, m_DownloadingTask.Count, m_WaitingTask.Count));
