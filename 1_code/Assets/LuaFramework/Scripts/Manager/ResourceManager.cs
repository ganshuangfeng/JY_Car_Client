...
namespace LuaFramework
{
    ...
    public class ResourceManager : Manager
    {
		...
		private string[] BASIC_DIR = new string[] {"common", "framework", "sproto", "gamecommon"};
		...
		//xxx#a#b# / xxx$3$4$ / #a#b#xxx / xx#a#b#cxx ...
		private bool IsContainSymbol(string value) {
		...
		private string[] FormatSymbol(string value) {
			string[] items = { };
			items = value.Split ('#');
			if (items.Length == 4)
				return FormatSymbolLetter (value, items);
			items = value.Split ('$');
			if (items.Length == 4)
				return FormatSymbolNumber (value, items);

			return new string[] {value};
		}

		private string[] FormatSymbolNumber(string value, string[] items) {
			int min, max;
			if (!int.TryParse (items [1], out min))
				return new string[] {value};
			if (!int.TryParse (items [2], out max))
				return new string[] {value};
			
			List<string> result = new List<string> ();
			for (int idx = min; idx <= max; ++idx) {
				result.Add (items [0] + idx.ToString () + items [3]);
			}
			return result.ToArray ();
		}
		private string[] FormatSymbolLetter(string value, string[] items) {
			char min, max;
			if (!char.TryParse (items [1], out min))
				return new string[] {value};
			if (!char.TryParse (items [2], out max))
				return new string[] {value};

			List<string> result = new List<string> ();
			for (int idx = min; idx <= max; ++idx) {
				result.Add (items [0] + (char)idx + items [3]);
			}
			return result.ToArray ();
		}
		...
		private bool ReadAssetTable(string sceneName) {
			...
			if (tableConfig == null) {
				Debug.LogError(string.Format("[AssetBundle] ReadAssetTable({0}) failed. Parse failed.", sceneName));
				return false;
			}
			...
			for (int idx = 0; idx < tableConfig.assetConfig.Count; ++idx) {
				...
				if (assetUnitCfg.lifeType > 0) {
					if (m_AssetTable.TryGetValue (assetUnitCfg.assetName, out assetUnit)) {
						if ((int)assetUnit.lifeType != assetUnitCfg.lifeType)
							Debug.LogError (string.Format ("[AssetBundle] ReadAssetTable({0}) exception: asset({1}) life({2} : {3}) conflict.",
								sceneName, assetUnitCfg.assetName, assetUnitCfg.lifeType, assetUnit.lifeType));
					} else {
						if (IsContainSymbol (assetUnitCfg.assetName)) {
							foreach (string item in FormatSymbol(assetUnitCfg.assetName))
								m_AssetTable.Add (item, new AssetUnit ((AssetLife)assetUnitCfg.lifeType));
						} else
			...
			for (int idx = 0; idx < tableConfig.preload.Count; ++idx) {
				assetName = tableConfig.preload [idx];
				if (IsContainSymbol (assetName)) {
					foreach (string item in FormatSymbol(assetName))
						m_PreloadAssetList.Add (item);
				} else
					m_PreloadAssetList.Add (assetName);
			}
		...
		public void LoadSceneLuaBundle(string sceneName) {
			...
			/*string dependLuaBundle = FindDependLuaBundle (sceneIdent);
			if (!string.IsNullOrEmpty (dependLuaBundle)) {
				LuaManager.RegisterBundle (dependLuaBundle);
			}*/
		...
		public void SetupManifest() {
			...
			if (!UseAssetBundle ()) {
				...
				#if UNITY_EDITOR
				Debug.Log("<color=red>XXXXXXXXXXXXXXXXX当前渠道=" + AppDefine.CurQuDao + "</color>");
				if (AppDefine.CurQuDao != "main")
				{
					ntaFile = Application.dataPath + "/" + NAME_TO_ASSET + "_" + AppDefine.CurQuDao + ".txt";
					lines = File.ReadAllLines (ntaFile);
					for (int idx = 0; idx < lines.Length; ++idx) {
							item = lines[idx].Split('|');
							if(item.Length != 2) {
        ...
        {
            string cacheScene = sceneName;
            if (!Path.HasExtension(sceneName))
                sceneName += ".unity";

			UnityEngine.Debug.Log("LoadSceneAsync System.DateTime.Now");
			UnityEngine.Debug.Log(System.DateTime.Now);

            ReadAssetTable(sceneName);
			UnityEngine.Debug.Log("ReadAssetTable System.DateTime.Now");
			UnityEngine.Debug.Log(System.DateTime.Now);
            ...
            if (!m_AssetToBundles.TryGetValue(sceneName.ToLower(), out bundleName))
            {
            ...
            StartCoroutine(LoadBundleAsync(bundleName, true, (AssetBundle assetBundle) =>
            {
                if (luaFunc != null)
                {
					UnityEngine.Debug.Log("luaFunc System.DateTime.Now");
					UnityEngine.Debug.Log(System.DateTime.Now);
        ...
        public void LoadSceneFinish(string sceneName)
        {
            ...
            //GaManager.LoadSceneFinish();

            UnloadBundle(bundleName);
        }
        //interface
        public void LoadScene(string sceneName, Action<string> callback) {
			if (!Path.HasExtension (sceneName))
				sceneName += ".unity";
		...
		public void LoadSceneAdditive(string sceneName, Action<string> callback) {
			if (!Path.HasExtension (sceneName))
				sceneName += ".unity";
			...
			if(!UseAssetBundle()) {
				SceneManager.LoadScene(FormatSceneName(sceneName), LoadSceneMode.Additive);
		...
		public string FormatSceneName(string sceneName)
        {
            ...
            if (UseAssetBundle ())
				...
				if (m_NameToAssets.TryGetValue (sceneName.ToLower(), out fileName))
					return fileName;
				else
		...
		public IEnumerator LoadAssetAsync<T> (string assetName, Action<T> sharpFunc, LuaFunction luaFunc) where T : UObject
		{
			...
			AssetUnit assetUnit;
			if(m_AssetTable.TryGetValue(assetTableKey, out assetUnit)) {
				if (assetUnit.templete != null) {
					//callback
					if(sharpFunc != null)
			...
			if(!UseAssetBundle()) {
				string fileName = string.Empty;
				if (m_NameToAssets.TryGetValue (assetName.ToLower(), out fileName))
					assetName = fileName;
				else
			...
			string bundleName = string.Empty;
			if (!m_AssetToBundles.TryGetValue (assetName.ToLower(), out bundleName)) {
		...
		private IEnumerator LoadDependenciesAsync(string bundleName, Action<AssetBundle> callback) {
			UnityEngine.Debug.Log("LoadDependenciesAsync System.DateTime.Now + bundleName " + bundleName);
			UnityEngine.Debug.Log(System.DateTime.Now);
		...
		{
			UnityEngine.Debug.Log("LoadBundleAsync System.DateTime.Now + bundleName " + bundleName);
			UnityEngine.Debug.Log(System.DateTime.Now);
			...
			UnityEngine.Debug.Log("LoadBundleAsync 2 System.DateTime.Now + bundleName " + bundleName);
			UnityEngine.Debug.Log(System.DateTime.Now);
			...
			UnityEngine.Debug.Log("LoadBundleAsync 3 System.DateTime.Now + bundleName " + bundleName);
			UnityEngine.Debug.Log(System.DateTime.Now);
