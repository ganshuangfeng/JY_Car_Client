...
*/
	...
	public class GameManager : Manager
	{
		...
		public int GetDebugMask()
		{
			int mask = debug_mask;
			if (m_RemoteVersionMap != null)
				mask |= m_RemoteVersionMap.debug_mask;
			return mask;
		}
		public bool IsNetDebug()
		{
			return (GetDebugMask() & 0X00000001) > 0;
		}
		public bool IsNetDetailDebug()
		{
			return (GetDebugMask() & 0X00000002) > 0;
		}
		public bool IsUpdDebug()
		{
			return (GetDebugMask() & 0X00000008) > 0;
		}
		...
		public string getEmbedChannel() {
			#if UNITY_EDITOR
				return AppDefine.CurEmbed;
			#endif

			return AppPredefine.EmbedChannel;
		}
		public string getMarketChannel() {
			#if UNITY_EDITOR
			if(string.IsNullOrEmpty(AppPredefine.MarketChannel)) {
			...
			#endif
			return AppPredefine.MarketChannel;
		}
		public string getMarketPlatform() {
			#if UNITY_EDITOR
			if(string.IsNullOrEmpty(AppPredefine.MarketPlatform)) {
				for(int idx = 0; idx < ChannelTal.Length; ++idx) {
					if(string.Compare(AppDefine.CurQuDao, ChannelTal[idx, 0]) == 0)
			...
			#endif
			return AppPredefine.MarketPlatform;
		}
		...
		IEnumerator Start ()
		{
			...
			if (ResManager.UseAssetBundle ()) {
				...
				{
					string embedChannel = getEmbedChannel();
					if(!string.IsNullOrEmpty(embedChannel))
						m_LoadingPanel = SetupPanel<LoadingPanel>("LoadingPanel_" + embedChannel, true);
					if (m_LoadingPanel == null) {
						...
						Debug.Log ("[GameManager] SetupPanel use embedchannel:" + embedChannel);
				}
		...
		void Update ()
		{
			...
			debug_mask = PlayerPrefs.GetInt("_DEBUG_CORE_", 0);

			if (m_GameState == (int)GameState.RUNNING)
			{
				if(IsUpdDebug())
					Debug.Log("[GameManager][OnLineUpdDebug] update gamestate is running");
				return;
			}

			if (IsUpdDebug())
				Debug.Log("[GameManager][OnLineUpdDebug] update gamestate is:" + (m_GameState >> 16) + ":" + (m_GameState & 0X0000FFFF));
			...
			if (IsCheckVersion () && (m_GameState & 0X0000FFFF) >= (int)GameState.REQUEST_VERSION) {
				if (DldManager.CheckNetValid())
				{
					...
					if(IsNetDetailDebug())
						Debug.Log("[GameManager][OnLineNetDebug] network is OK");
				}
				else
				{
					if (IsNetDetailDebug())
						Debug.Log("[GameManager][OnLineNetDebug] network is break");
		...
		private IEnumerator PlayingCinematic(Action<bool> callback) {
			...
			string localVideoFile = m_LocalPath + CINEMATIC_FILE;
			string localChannelVideoFile = m_LocalPath + CINEMATIC_CHANNEL + getEmbedChannel () + ".mp4";

			int extractVideoCount = 0;
			if (m_FirstRun || (!File.Exists (localVideoFile) && !File.Exists(localChannelVideoFile))) {
				extractVideoCount = 2;
				StartCoroutine (ResManager.ReadFile (m_StreamingPath + CINEMATIC_CHANNEL + getEmbedChannel () + ".mp4", ((bool isOK, byte[] datas, string errMsg) => {
					--extractVideoCount;
					if (isOK)
						ResManager.WriteFile(localChannelVideoFile, datas, 0, datas.Length);
					else
						;
				})));
				StartCoroutine (ResManager.ReadFile (m_StreamingPath + CINEMATIC_FILE, ((bool isOK, byte[] datas, string errMsg) => {
					--extractVideoCount;
					if (isOK)
						ResManager.WriteFile (localVideoFile, datas, 0, datas.Length);
					else {
						m_GameState = ((int)GameState.PLAY_CINEMATIC << 16) | (int)GameState.EXCEPTION;
						Debug.LogError (string.Format ("[AssetBundle] PlayingCinematic failed: Read Cinematic File({0}) failed({1}).", m_StreamingPath + CINEMATIC_FILE, errMsg));
					}
				})));
			}
			...
			if (File.Exists (localChannelVideoFile)) {
				localVideoFile = localChannelVideoFile;
				Debug.Log ("[Debug] video channel:" + localVideoFile);
			}

			Debug.Log ("[Debug] extractVideoCount:" + extractVideoCount + ", " + localVideoFile);
		...
		*/
			...
			StartCoroutine(CheckVersionMap(rvmFile, svr_lists, (vm) => {
				...
				if(vm != null) {
					...
					if(result < 0) {
						...
						string gamecommon = getLocalPath("gamecommon/gamecommon_prefab.unity3d");
						if(result == -3 && !File.Exists(gamecommon)) {
							ShowHintNoticePanel("下载最新版本，全新体验升级", ()=>{
								string appurl = ResManager.LoadText("App_URL.txt", null);
								if(!string.IsNullOrEmpty(appurl)) {
									AppURL url = JsonUtility.FromJson<AppURL>(appurl);
									switch (Application.platform)
									{
									case RuntimePlatform.Android:
										Application.OpenURL(url.android);
										break;
									case RuntimePlatform.IPhonePlayer:
										Application.OpenURL(url.ios);
										break;
									default:
										Debug.Log("[AssetBundle] CompareVersion failed, need reinstall. but platform is invalid:" + Application.platform.ToString());
										break;
									}
								}
		...
		private void EnterGame() {
			...
			StartCoroutine (RunningGame ((bool isOK) => {
				...
				if (IsBasicIdent ()) {
					...
					ResManager.AddToAssetTable("NetwordWaitScene.prefab", AssetLife.AL_RESIDENT);
		...
		IEnumerator TestDownload() {
			...
			fileMap.Add ("gamecommon/file_list_remote.txt", new VersionFile ("gamecommon/" + FILE_LIST));
