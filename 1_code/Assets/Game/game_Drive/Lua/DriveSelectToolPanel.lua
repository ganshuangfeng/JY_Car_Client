...
DriveSelectToolPanel = basefunc.class()
local M = DriveSelectToolPanel
M.name = "DriveSelectToolPanel"
...
function M:MyRefresh(player_op)
	...
	self.title_img.sprite = GetTexture(DriveMapManager.GetMapAssets("zd_icon_bj"))

    for k, v in ipairs(self.for_select_vec) do
        local cfg = ToolsManager.GetToolsCfgById(v)
        if cfg then
...
function M:OnSelect(op_arg_1)
    ...
    if op_arg_1 and self.select_objs[op_arg_1] then
        DriveModel.SendRequest("drive_game_player_op_req", {op_type = DriveModel.OPType.select_index, op_arg_1 = op_arg_1})
    end
end
