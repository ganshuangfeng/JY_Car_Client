...
SkillSun = basefunc.class(SkillBase)
...
function C:ctor(skill_data)
    SkillSun.super.ctor(self,skill_data)
end
...
function C:OnTriggerMain()
    ...
    if self.obj_datas and next(self.obj_datas) then
        ...
        DriveAnimManager.PlayNewAttributeChangeFx("normal_art_font_fx","com_img_ld_qt","",true,self.launcher_car:GetCenterPosition(),function()
            self:OnTriggerEnd()
        end,true)
    end
end