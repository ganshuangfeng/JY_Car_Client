...
NetwordWaitNode = basefunc.class()
...
function NetwordWaitNode.Create(parent,tag,offsetPos,scale)
	if tag then
	    NetwordWaitNode.RemoveByTag(tag)
	end
    return NetwordWaitNode.New(parent,tag,nil,offsetPos,scale)
end


function NetwordWaitNode.CreateBySize(parent,tag,size,offsetPos,scale)
	if tag then
	    NetwordWaitNode.RemoveByTag(tag)
	end
    return NetwordWaitNode.New(parent,tag,size,offsetPos,scale)
end


function NetwordWaitNode.RemoveByTag(tag)
    local jh = jhTags[tag]
...
function NetwordWaitNode.RemoveAll()
...
local jhScale=0.8
function NetwordWaitNode:ctor(parent,tag,size,offsetPos,scale)
	self.tag = tag

	offsetPos = offsetPos or {x=0,y=0}
	scale = scale or jhScale

	self.UIEntity = newObject("NetwordWaitNode", parent.transform.parent)
...
--移除
function NetwordWaitNode:Remove()
