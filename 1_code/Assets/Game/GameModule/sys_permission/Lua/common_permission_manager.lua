...
function C.init( _is_client , _service_cfg , _use_for )
	...
	if C.is_for_server then
		...
		-- 客户端自己加载权限配置文件
		C.load_config( GameModuleManager.ExtLoadLua("sys_permission", "permission_server_config") )
	...
	elseif _use_for == "client" then
		C.asset_judge_module = GameModuleManager.ExtLoadLua("sys_permission", "permission_asset_judge_module_for_center_client")
	else
		C.asset_judge_module = GameModuleManager.ExtLoadLua("sys_permission", "permission_asset_judge_module_for_center_client")
	end
end
