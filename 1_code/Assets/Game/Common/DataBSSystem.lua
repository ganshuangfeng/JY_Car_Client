...
local cfg = HotUpdateConfig("Game.Common.data_bs_system_config").main
DataBSSystem = {}
local M = DataBSSystem
...
local start_num		-- = UnityEngine.PlayerPrefs.GetInt("DataBSSystem",0)
local login_id

local listener
local function AddListener()
    listener={}
    listener["dbss_send"] = M.dbss_send
    listener["dbss_send_power"] = M.dbss_send_power
    listener["dbss_send_e"] = M.dbss_send_e
    listener["EnterScene"] = M.EnterScene
	listener["ExitScene"] = M.ExitScene
	listener["EnterForeGround"] = M.EnterForeGround
    listener["EnterBackGround"] = M.EnterBackGround
    listener["network_connect"] = M.ServerConnectSucceed
    listener["network_exception"] = M.network_exception
    listener["network_disconnect"] = M.network_disconnect
...
function M.Init()
	...
	start_num = UnityEngine.PlayerPrefs.GetInt("DataBSSystem",0)

	local function GetLocalLoginData(name)
		local path
		if AppDefine.IsEDITOR() then
			path = Application.dataPath .. "/" .. name .. ".json"
		else
			path = AppDefine.LOCAL_DATA_PATH .. "/" .. name .. ".txt"
		end
		if File.Exists(path) then
			local str = File.ReadAllText(path)
			local login_data = json2lua(str)
			if login_data and next(login_data) then
	...
	local function GetLoginID()
		...
		dump(login_id, "<color=green>DataBSSystem login_id</color>")
		return login_id
	end
	login_id = GetLoginID()
...
function M.Send()
	...
	Network.SendPostBSDS(data, function (code)
		code = code or ""
		print("<color=white>SendPostBSDS A Result</color>",code)
	end)
	s_t = os.time()
	M.val_list = {}
end
...
function M.dbss_send(data)
	M.Add(data.key)
end

function M.dbss_send_power(data)
...
function M.EnterForeGround()
	M.dbss_send_power({key = "enter_fore_ground"})
end

function M.EnterBackGround()
	M.dbss_send_power({key = "enter_back_ground"})
end

function M.ServerConnectSucceed()
	M.dbss_send_power({key = "server_connect_succeed"})
end

function M.network_exception()
	M.dbss_send_power({key = "server_connect_exception"})
end

function M.network_disconnect()
	M.dbss_send_power({key = "server_connect_disconnect"})
end

--lua层更新完成后调用
function M.dbss_send_e()
	start_num = start_num or 0
	start_num = start_num + 1
	UnityEngine.PlayerPrefs.SetInt("DataBSSystem",start_num)
	SN = SN + 1
	local _sn = SN
