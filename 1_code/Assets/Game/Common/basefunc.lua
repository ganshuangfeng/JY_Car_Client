...
--------------------------------- add by ryx
---用Lua实现的GeneratingVar
function basefunc.GeneratingVar(transform,lua_table)
...
function basefunc.add_click(go,func)
	go.transform:GetComponent("Button").onClick:AddListener(function(  )
		func()
	end)
end

---字符串替换 按配置表获取table中的属性
function basefunc.format_extend(target_str,get_config,format_table)
	local ret = target_str
	for k,v in pairs(get_config) do
		--k：target_str中的标识符
		--v：format_table中的属性
		local format_value = format_table[v]
		if type(format_value) == "table" then
			for _k,_v in ipairs(format_value) do
				ret = string.gsub(ret,k,_v,1)
			end
		elseif type(format_value) == "string" or type(format_value) == "number" then
			ret = string.gsub(ret,k,format_value,1)
		else
			-- dump(v,"<color=red>不满足条件的属性值</color>")
			-- dump(format_table,"<color=red>不满足条件的format_table</color>")
		end
	end
	return ret
end
