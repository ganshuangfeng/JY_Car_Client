...
public class ADRewardMgr {
	private static ADRewardMgr m_instance;
	public static ADRewardMgr Instance {
		get {
			if (m_instance == null)
				m_instance = new ADRewardMgr ();
			return m_instance;
		}
	}
	...
	internal class Unit
	{
		...
		public void Prepare(string rewardName, int rewardAmount, string userID, string extraData, int width, int height, LuaFunction callback) {
			var adSlot = new AdSlot.Builder ()
				...
				.Build ();
			#if UNITY_IOS
				ADRewardMgr.Instance.adNative.LoadExpressRewardAd (adSlot, new RewardVideoAdListener (this));
			#else
				ADRewardMgr.Instance.adNative.LoadRewardVideoAd (adSlot, new RewardVideoAdListener (this));
			#endif
		}
