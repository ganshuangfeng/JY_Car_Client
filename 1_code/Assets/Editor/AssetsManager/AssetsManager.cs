...
public class AssetsManager : EditorWindow
{
    ...
    public class AssetsManagerStruct
    {
        ...
        public Dictionary<string, bool> gamemodule_config;
        public Dictionary<string, bool> gamemodule_folder;
    }
    ...
    private static void LocationListSort(List<Location> list)
    {
        ...
        list.Reverse();//这里倒序，以game中为基础，gamemodule中进行copy
    }
    ...
    private static string LocationToNewPath(Location l, AssetDescriptionInfo adi)
    {
        ...
        if (l.location2 == "GameModule") newName += "_" + l.location3;
    ...
    private static void SetGameModuleConfigStruct()
    {
        CreateGameModuleConfig();
        Dictionary<string, bool> gamemodule_config = new Dictionary<string, bool>();
        Dictionary<string, bool> gamemodule_folder = new Dictionary<string, bool>();
        string path = GameModuleConfigPath;//以主版本活动为准
        string json_str = File.ReadAllText(path);
        JsonData jsonData = JsonMapper.ToObject(json_str);
        JsonData modules = jsonData["modules"];
        foreach (JsonData item in modules)
        {
            ...
            if (!gamemodule_config.ContainsKey(pathArr[1]))
            {
                gamemodule_config.Add(pathArr[1], enable.ToString().ToLower() == "true");
            }
        }

        string[] subFolders = AssetDatabase.GetSubFolders("Assets/Game/GameModule");
        foreach (var _path in subFolders)
        {
            string[] strArr = _path.Split(new string[] { "/" }, StringSplitOptions.RemoveEmptyEntries);
            string str = strArr[3];
            if (gamemodule_config.ContainsKey(str))
            {
                gamemodule_folder.Add(str, gamemodule_config[str]);
            }
            else
            {
                gamemodule_folder.Add(str, false);
            }
        }

        asset_manager_struct.gamemodule_config = gamemodule_config;
        asset_manager_struct.gamemodule_folder = gamemodule_folder;
    }
    ...
    private static void SetLuaCodeReferences()
    {
        ...
        foreach (var item in asset_guid)
        {
            ...
            if (item.Value.name == "SceneConfig") continue;
    ...
    private static void CalculateDepend(ref List<Location> location_list, ref List<Location> game_location, ref List<Location> normal_location, ref List<Location> gamemodule_location, AssetDescriptionInfo adi)
    {
        location_list.Clear();
        ...
        foreach (var location in gamemodule_location) LocationListAdd(location_list, location);
        ...
        for (int i = 0; i < location_list.Count; i++)
        {
            ...
            if (orig == "GameModule") orig = location.location3;
        ...
        foreach (var depend in depend_list)
        {
            ...
            if (asset_manager_struct.gamemodule_config.ContainsKey(depend))
            {
                depend_location.location2 = "GameModule";
                ...
                if (asset_manager_struct.depend.ContainsKey(orig))
                {
                    ...
                    foreach (var depend in depends)
                    {
                        for (int j = location_list.Count - 1; j >= 0; j--)
                        {
                            ...
                            if (m_location == "GameModule") m_location = check_location.location3;
                            //在依赖的地方，是依赖关系，不用移动
    ...
    private static void CalculateLocationReferences()
    {
        ...
        foreach (var item in asset_guid)
        {
            ...
            List<Location> gamemodule_location = new List<Location>();
            //depend 依赖处理
            foreach (var location in location_list)
            {
                if (location.location1 == "Game" && asset_manager_struct.game_scene.ContainsKey(location.location2))
                    ...
                    LocationListAdd(normal_location, location);
                else if (location.location1 == "Game" && location.location2 == "GameModule" && asset_manager_struct.gamemodule_config.ContainsKey(location.location3))
                    LocationListAdd(gamemodule_location, location);
            }
            //不需要移动
            ...
            if (location_list.Count == 1)
            {
                ...
                //depend 依赖处理
                CalculateDepend(ref location_list, ref game_location, ref normal_location, ref gamemodule_location, item.Value);
                //处理完后，先排序
            ...
            foreach (var location in location_list)
            {
                if (location.location1 == "Game" && location.location2 == "GameCommon")
                {
                    LocationListAdd(item.Value.locationRealeList, location);
                    b = true;
                    break;
    ...
    private static void AddCopyLog(AssetDescriptionInfo adi, Location newLocation, ref int count)
    {
        ...
        if (adi.locationCodeList.Contains(newLocation))
        {
            ...
            else
            {
                ...
                if (location_str == "GameModule") location_str = newLocation.location3;
                    ...
                    foreach (var item in asset_manager_struct.depend)
                    {
                        foreach (var dep in item.Value)
                        {
                            if (dep == location_str)
                            {
                                Location l = new Location();
                                l.location1 = "Game";
                                if (asset_manager_struct.gamemodule_config.ContainsKey(item.Key))
                                {
                                    //活动
                                    l.location2 = "GameModule";
    ...
    private static void SaveReplaceGuid(AssetDescriptionInfo adi, Location newLocation, string newPath)
    {
        ...
        if (depend == "GameModule") depend = newLocation.location3;
    ...
    public static string GameModuleConfigPath = "Assets/VersionConfig/GameModuleConfig.json";
    public static string ActConfigPath = "Assets/game_module_show_config.txt";
    public static string GameModuleConfig = "Assets/Game/GameCommon/Lua/game_module_config.lua";
    [MenuItem("Assets/Util/生成活动配置", false, 10)]
    [MenuItem("Util/生成活动配置", false, 10)]
    public static void CreateGameModuleConfig()
    {
        string GameModuleConfigPath = "Assets/VersionConfig/GameModuleConfig.json";
        string ActConfigPath = "Assets/game_module_show_config.txt";
        string GameModuleConfig = "Assets/Game/GameCommon/Lua/game_module_config.lua";
        Debug.Log(AppDefine.CurQuDao);
        if (AppDefine.CurQuDao != "main")
        {
            GameModuleConfigPath = "Assets/VersionConfig/GameModuleConfig_" + AppDefine.CurQuDao + ".json";
            ActConfigPath = "Assets/game_module_show_config_" + AppDefine.CurQuDao + ".txt";
        ...
        _dic.Add("modules", new List<act_struct>());
        foreach (var item in dic)
        {
            bool is_enable = item.Value["enable"] == "1" ? true : false;
            act_struct act_st = new act_struct()
            {
                name = "GameModule/" + item.Value["key"],
                enable = is_enable,
            };

            _dic["modules"].Add(act_st);
        }
        string json_str = JsonMapper.ToJson(_dic);
        Debug.Log("GameModuleConfig " + json_str);
        WriteAllText(GameModuleConfigPath, json_str);
        //生成game_module_show_config
        string txt_str = "";
        foreach (var item in dic)
        {
            txt_str += item.Value["key"] + "|" + item.Value["desc"] + "|" + item.Value["state"] + "\n";
        }
        Debug.Log("game_module_show_config " + txt_str);
