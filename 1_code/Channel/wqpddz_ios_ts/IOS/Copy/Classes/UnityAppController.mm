...
#define WeiXinID @"wxa67208ed9db78f10" 
...
extern "C" {
    ...
    void Handle813819(const char *json_data) {
        SimpleFeedback("InitResult", 0, 0);
    }
    ...
    void Handle11146813(const char *json_data) {
        if(![WXApi isWXAppInstalled]) {
		SimpleFeedback("LoginResult", -2, 0);
        } else {
		gIsRelogin = false;
    ...
    void Handle11146813142019(const char *json_data) {
        SimpleFeedback("LoginOutResult", 0, 0);
    }
    ...
    void Handle17411146813(const char *json_data) {
    	gIsRelogin = true;
        [[NSNotificationCenter defaultCenter] postNotificationName:sendAuthRequestNotification object:nil];
    }
    ...
    void Handle15024(const char *json_data) {
        SimpleFeedback("PayResult", 0, 0);
    }
    ...
    void Handle1870174(const char *json_data) {
        WXShare(json_data);
    }
    ...
    void Handle187142202214201319241319417(const char *json_data) {
        SimpleFeedback("ShowAccountCenterResult", 0, 0);
    }
    ...
    void Handle18419201503(const char *json_data) {
        NSString *strJson = [[NSString alloc] initWithUTF8String:json_data];
        NSData *jsonData = [strJson dataUsingEncoding:NSUTF8StringEncoding];
        NSDictionary *dic = [NSJSONSerialization JSONObjectWithData:jsonData options:NSJSONReadingMutableLeaves error:nil];
        NSString *adAppId = [dic objectForKey:@"appId"];
    ...
    const char* Push3421824191410413() {
        if(![gStrPushDeviceToken isEqual:[NSNull null]]) {
        	const char *raw = [gStrPushDeviceToken UTF8String];
                char *mem = (char*)malloc(strlen(raw) + 1);
                strcpy(mem, raw);
	...
	NSMutableString *str=[[NSMutableString alloc]initWithFormat:@"tel:%@",strNumber];
	UIWebView *callWebview = [[UIWebView alloc] init];
	[callWebview loadRequest:[NSURLRequest requestWithURL:[NSURL URLWithString:str]]];
	[GetAppController().rootView addSubview:callWebview];
    }
    void Phone2011112015(const char *number) {
        NSString *strNumber = [[NSString alloc] initWithUTF8String:number];
	NSMutableString *str=[[NSMutableString alloc]initWithFormat:@"tel:%@",strNumber];
	UIWebView *callWebview = [[UIWebView alloc] init];
	[callWebview loadRequest:[NSURLRequest requestWithURL:[NSURL URLWithString:str]]];
    ...
    void Querying281924130124(float latitude, float longitude) {
        AppController_Querying281924130124(latitude, longitude);
    }
    ...
    int Start1742141738136(const char *fileName) {
        NSString *param = [[NSString alloc] initWithUTF8String:fileName];
        //return AppController_StartRecording(param);
        gAmrRecord = param;
        return [AudioUtil start0203814174214173];
    }
    ...
    void Stop1742141738136(BOOL callback) {
        //AppController_StopRecording(callback);
        [AudioUtil stop0203814174214173];
    ...
    int Playing174214173(const char *fileName) {
        NSString *param = [[NSString alloc] initWithUTF8String:fileName];
        return [AudioUtil play174214173:param];
    }

    void StopPlayingRecord() {
        [AudioUtil stopPlayRecord];
    }
    void Stop15110248136174214173() {
        [AudioUtil stop1511024174214173];
    }

    void ShowProductRating() {
    }
    void Show151714320219170198136() {
    }
    ...
    int Can1114201981413() {
    	//return getCanIosLocation();
        return [[MyCLLocationManager shareManager] canLocation];
    }

    int CanVoice() {
        return getCan814182114824();
    }
    int Can2114824() {
    ...
    int CanCamera(bool deep) {
        return getCan8141820124170();
    }
    int Can20124170(bool deep) {
        return getCanIosCamera();
    }

    int CanPushNotification() {
        return getCan1520187131419858201981413();
    }
    int Can1520187131419858201981413() {
    ...
    void Opening20124170() {
        openIosCamera();
    }

    void GoingSetScene(const char *mode) {
        goto184191824134(mode);
    }
    void Going184191824134(const char *mode) {
        gotoSetScene(mode);
    }

    void ForceQuiting() {
    }
    void Force16208198136() {
    }
}
...
- (BOOL)application:(UIApplication*)application didFinishLaunchingWithOptions:(NSDictionary*)launchOptions
{
    ...
    [UMConfigure initWithAppkey:@"5df0ac680cafb2f8d7000350" channel:@"App Store"];
...
void AppController_Querying281924130124(float inlatitude, float inlongitude)
{
    CLLocation *location = [[CLLocation alloc] initWithLatitude:inlatitude longitude:inlongitude];
...
int AppController_Start1742141738136(NSString *fileName)
{
    return [GetAppController() StartRecording:fileName];
}
...
void AppController_Stop1742141738136(BOOL callback)
{
    [GetAppController() StopRecording:callback];
}
