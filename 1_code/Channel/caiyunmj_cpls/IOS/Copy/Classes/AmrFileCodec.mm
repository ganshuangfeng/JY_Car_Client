...
BOOL fuckAndroid3GP(NSData *data, NSMutableData *result) {
	...
	//data = [[NSMutableData alloc]initWithCapacity:fullAmrDataLength];
    [result appendBytes:AMR_MAGIC_HEADER length:6];
    [result appendBytes:bis length:rawAmrDataLength];
...
BOOL DecodeAmrDataToWAVE(NSMutableData* output, NSData* input) {
    ...
    const char *rfile = (const char*)[input bytes];
    if(strncmp(rfile, AMR_MAGIC_NUMBER, strlen(AMR_MAGIC_NUMBER))) {
        NSMutableData *result = [[NSMutableData alloc]init];
        if(!fuckAndroid3GP(input, result))
            return false;
        input = result;
    }
    int maxLen = [input length];
    int pos = 0;
    rfile = (const char*)[input bytes];
    ...
    int stdFrameSize;
    int nTemp = ReadAMRFrameFirstData((char*)rfile,pos,maxLen, amrFrame, &stdFrameSize, &stdFrameHeader);
    if (nTemp==0) {
...
BOOL EncodePCMToAMR(NSString *outputFile, char* data, int maxLen,int nChannels, int nBitsPerSample)
{
    char* oldBuf = data;
    short speech[160];
    ...
    [output appendBytes:AMR_MAGIC_NUMBER length:strlen(AMR_MAGIC_NUMBER)];
    ...
    while(1)
    {
        ...
        [output appendBytes:amrFrame length:byte_counter];
    }
...
/*#ifdef DEBUG
    ...
    [output writeToFile:wavFile atomically:YES];
#endif*/
...
BOOL EncodeWAVEToAMR(NSString *outputFile, NSData* input, int nChannels, int nBitsPerSample)
{
    ...
    return EncodePCMToAMR(outputFile, buf, maxLen- nPos, nChannels, nBitsPerSample);
}
